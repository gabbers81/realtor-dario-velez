...
- July 3, 2025. **CRITICAL PDF Production Issue RESOLVED**: Completed comprehensive fix for Secret Garden and Solvamar Macao PDF failures in production. Root causes identified and fixed: (1) Missing production files - PDFs weren't copied to `dist/public/pdfs` during build, (2) File permissions - secret-garden.pdf had 600 permissions preventing access, (3) Large file handling - 40MB and 42MB files hit server memory limits. Solution: Enhanced PDF serving with streaming support for files >35MB, HTTP 206 range request support, proper error handling, manual production file copying process, and comprehensive diagnostic tools. All 8 PDFs now serve correctly in production with optimized performance for large files.
