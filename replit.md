...
- July 3, 2025. **Large PDF Production Issue Resolved**: Implemented comprehensive solution for 40MB+ PDF display issues (Secret Garden & Solvamar) in production. Changes:
  - **Server-side**: Added intelligent streaming for files >30MB with 64KB chunks optimized for HTTP/2 compatibility
  - **Frontend**: Implemented fallback UI for large PDFs that displays download/open buttons instead of attempting iframe embed
  - **Translation Support**: Added multilingual messages explaining large PDF handling
  - **Root Causes Addressed**: HTTP/2 frame size limits, server memory constraints, and iframe security policies
